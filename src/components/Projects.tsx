...
const projects: Project[] = [
  {
    id: 1,
    title: 'AgentVerse',
    description: 'Watch your AI agent fight. Turns an AI agent\'s real problem-solving workflow into a live, side-scrolling pixel-art JRPG battle — the agent is the hero, your task is the boss, and every tool call becomes a skill cast. Powered by Grok + the X (xAI) API.',
    tech: ['TypeScript', 'React', 'Zustand', 'Grok API'],
    status: 'live',
  ...
  },
  {
    id: 2,
    title: 'NoteWise',
    description: 'AI-based web app for creating mind graphs and notes from texts — turns raw reading material into structured, connected knowledge.',
  ...
  },
  {
    id: 3,
    title: 'RoomBotSim',
    description: 'Four phone photos become a walkable simulation of your room. Drop a robot in, give it a task in plain language, and every attempt — including the failures — becomes training data.',
    tech: ['Python', 'MuJoCo', 'FastAPI', 'World Labs API'],
    status: 'live',
    github: 'https://github.com/Vivo50E/RoomBotSim',
    thumbnail: '/projects/roombotsim.png',
    featured: true,
  },
  {
    id: 4,
...
function ProjectCard({ project, index, isInView }: { project: Project; index: number; isInView: boolean }) {
  ...
  return (
    <motion.div
      ...
      className={`hud-panel group relative p-6 border bg-zinc-900/20 hover:bg-zinc-900/50 transition-all duration-300 flex flex-col ${
        project.featured
          ? 'border-amber-500/30 hover:border-amber-500/50 hover:shadow-[0_0_30px_rgba(251,191,36,0.1)]'
          : 'border-zinc-800 hover:border-zinc-700'
      }`}
    >
      {project.featured && (
        <div className="absolute -top-2.5 left-5 z-10">
          <span className="px-3 py-0.5 text-xs font-semibold bg-amber-500 text-black hud-label">Featured</span>
        </div>
      )}

      {project.thumbnail ? (
        <div className="-mx-6 -mt-6 mb-4 relative aspect-video overflow-hidden border-b border-zinc-800">
          <img
    ...
    <section id="projects" className="py-32 px-6" ref={ref}>
      <div className="max-w-6xl mx-auto">
        <motion.div
          ...
          className="mb-4"
        >
          <p className="hud-label text-amber-400 text-sm mb-3">Sector 03 · Mission Log</p>
          <h2 className="text-4xl md:text-5xl font-black text-white">Mission Log</h2>
        </motion.div>
        ...
        >
          ...
          <code className="font-mono text-cyan-400 text-sm bg-cyan-500/10 px-1 rounded">Projects.tsx</code>.
        </motion.p>
        ...
        <motion.div
          ...
          className="hud-panel mb-10 p-5 border border-zinc-800 bg-zinc-900/20"
        >
          ...
          <div className="flex flex-wrap gap-3">
            {[
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 px-4 py-2 border border-zinc-700 bg-zinc-800/50 hover:border-cyan-500/40 hover:bg-cyan-500/5 transition-all text-sm"
              >
